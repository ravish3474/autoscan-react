...
const editDealer = props => {
  ...
  const updateDealer = payload => {
    axios
      .patch(
        `${process.env.REACT_APP_API_URL}/dealer/update-dealer/${dealerId}`,
        payload,
        {
          headers: {
  ...
  const handleSubmit = event => {
    ...
    let { password, confirm_password, phone } = statePayload;
    ...
    payload.append("name", statePayload?.name);
    payload.append("email", statePayload?.email);
    payload.append("phone", statePayload?.phone);
    payload.append("address", statePayload?.address);
  ...
  const fetchDealerDetails = () => {
    ...
    axios
      .get(`${process.env.REACT_APP_API_URL}/dealer/fetch-dealerbyid/${dealerId}`)
      .then(response => {
        const { success, dealer } = response.data;
        if (success) {
          setStatePayload(dealer);
