...
const modelManagement = () => {
  ...
  const handleToggleStatus = async (modelId) => {
    axios
      .post(`${process.env.REACT_APP_API_URL}/model/toggle-status/${modelId}`)
      .then((response) => {
        ...
        fetchModelsList(1, sizePerPage);
        toast.success("Model status updated successfully");
      })
      .catch((err) => {
        console.log("Error while deleting the model list:::", err);
        toast.error("Error while deleting the model list", err);
      });
  };
  ...
  const handleModelDelete = (modelId) => {
    if (window.confirm("Are you sure you want to delete this Model?")) {
      axios
        .delete(
          `${process.env.REACT_APP_API_URL}/model/delete-model/${modelId}`
        )
        .then((response) => {
  ...
  const fetchModelsList = async (page, size) => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL}/model/list?page=${page}&pageSize=${size}`
      );
      const { model, total_counts } = response?.data;
      setModelsData(model);
      ...
      formatter: (cell, row) => {
        ...
        return (
          <div className="action-application">
            ...
            <div className="action-one">
              <Link
                to={`/edit-model-management/${row.id}`}
                title="Edit"
            ...
            <div
              ...
              onClick={() => handleModelDelete(row.id)}
  ...
  const handleTableChange = (type, { page, sizePerPage }) => {
    setTimeout(() => {
      fetchModelsList(page, sizePerPage);
      setCurrentPage(page);
    }, 500);
  };
  ...
  const handleCSVDownload = async (e) => {
    ...
    try {
      ...
      csvModelData = response?.data?.model;
    } catch (error) {
      console.error("Error fetching role data:", error);
    }
    ...
    let allData = JSON.parse(JSON.stringify(csvModelData)) || [];
  ...
  const renderCheck = (el) => {
    ...
    let searchItem = JSON.stringify({
      ...
      model_name: el?.model_name?.toLowerCase() || "",
    });
        ...
        <Container fluid>
          <Row>
            <div className="table-modeld">
              ...
              <RemotePagination
                // data={leadsData}
                data={modelsData?.filter(renderCheck)}
                columns={columns}
                page={currentPage}
