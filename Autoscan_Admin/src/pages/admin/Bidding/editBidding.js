...
const [chessis, setChessis] = useState(null);
  ...
  const createNewInspection = payload => {
    ...
    axios
    .patch(
      `${process.env.REACT_APP_API_URL}/brand/update-Inspection/${inspectionId}`,
      payload,
      {
        headers: {
  ...
  const fetchInspectionDetails = () => {
    ...
    axios
      .get(`${process.env.REACT_APP_API_URL}/inspection/fetch-Inspection/${inspectionId}`)
      .then(response => {
        const { success, inspection } = response.data;
        if (success) {
          setStatePayload(inspection);
